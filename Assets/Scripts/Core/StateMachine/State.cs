...
namespace Core.StateMachine
{
    ...
    public abstract class AbstractState<TEnum> : IState<TEnum> where TEnum : Enum
    {
        ...
        public void ChangeState(TEnum enumState, bool needEnter = true)
        {
            ...
            if (Verbose)
            {
                Debug.Log($"[AbstractState][ChangeState] Change state to: {enumState}");
            }
            ...
            if (needEnter)
            {
                if (Verbose)
                {
                    Debug.Log($"[AbstractState][ChangeState] Enter to state: {enumState}");
                }
                newState.Enter();
            }
        }
        ...
        public void Update(float deltaTime)
        {
            ...
            for (var i = 0; i < conditions.Count; i++)
            {
                if (conditions[i].Predicate())
                {
                    if (Verbose)
                    {
                        Debug.Log($"[AbstractState][Update] Condition passed");
                    }
                    conditions[i].Action();
                }
            }
        ...
        public void Exit()
        {
            if (onExit != null)
            {
                if (Verbose)
                {
                    Debug.Log($"[AbstractState][ChangeState] Exit from state");
                }
                onExit();
            }
        ...
        public void TriggerEvent(string name, EventArgs eventArgs)
        {
            ...
            if (events.TryGetValue(name, out myEvent))
            {
                if (Verbose)
                {
                    Debug.Log($"[AbstractState][TriggerEvent] event name: {name}");
                }
                myEvent(eventArgs);
            }
        }
