...
namespace GoogleSheetsToUnity
{
    ...
    {   
        /// <summary>
        /// Checks for a valid token and if its out of date attempt to refresh it
        /// </summary>
        /// <returns></returns>
        ...
        private static IEnumerator Read(UnityWebRequest request, GSTU_Search search, bool containsMergedCells, UnityAction<GstuSpreadSheet> callback)
        {
            ...
            using (request)
            {
                ...
                if (containsMergedCells)
                {
                    ...
                    var root = JSON.Load(request2.downloadHandler.text).Make<SheetsRootObject>();
                    response.sheetInfo = root.sheets.FirstOrDefault(x => x.properties.title == search.worksheetName);
                }

                callback?.Invoke(new GstuSpreadSheet(response, search.titleColumn,search.titleRow));
            }
        }
