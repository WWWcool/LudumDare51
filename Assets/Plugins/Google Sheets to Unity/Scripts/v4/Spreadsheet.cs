...
namespace GoogleSheetsToUnity
{
   ...
   public static GstuSpreadSheet CreateSheetFromString(string data)
        {
            string[] lines = data.Split("\n"[0]); 
            string[] titleRow = SplitCsvLine(lines[0]);
 
            Dictionary<string, List<GSTU_Cell>> rows = new Dictionary<string, List<GSTU_Cell>>();
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                var list = new List<GSTU_Cell>();
                for (int k = 0; k < row.Length; k++)
                {
                    list.Add(new GSTU_Cell(row[k], titleRow[k], i + 1));
                }
                rows.Add(row[0], list);
            }
            
            return new GstuSpreadSheet(rows);
        }
        ...
        public GstuSpreadSheet(Dictionary<string, List<GSTU_Cell>> rs)
        {
           Cells.Clear();
           ...
           foreach (var row in rs)
           {
               ...
               foreach (GSTU_Cell cell in dataValue)
               {
                   ...
                   cell.columnId = cell.Column();
                   cell.rowId = cell.Row().ToString();
